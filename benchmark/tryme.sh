...
echo "LIB/PQ: EXECUTING FOR LOCKS"
jq -ncM "$UPSERT_LOCK" \
...
echo "LIB/PQ: EXECUTING FOR UPSERT CONFLICT"
...
echo "LIB/PQ: EXECUTING FOR UPSERT DO NOTHING"
...
echo "LIB/PQ: EXECUTING FOR UPSERT CTE"
jq -ncM "$UPSERT_CTE" \
 | vegeta attack -format=json -duration=40s -connections=20 -rate=100 | vegeta encode \
 | vegeta report -type="hist[0,2ms,4ms,6ms,8ms,10ms,15ms]"
...
echo "LIB/PQ: EXECUTING FOR UPSERT CTE RANDOM"
